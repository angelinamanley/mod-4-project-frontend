...
class SideMenuContainer extends React.Component {
	...
	componentDidMount() {
        ...
        API.getUsers().then(users => this.setState({ usersList: users.filter(user => user.id !== this.props.currentUserId)}));
	...
	render() {
		return (
			<div className="side-menu-container">
                ...
                : 
				<ListDisplay addRoomViewOn={this.addRoomViewOn} setSelectedSessionAndRoomIds={this.props.setSelectedSessionAndRoomIds} usersList={this.state.usersList} roomsList={this.state.roomsList}/>
                }
                </div>
		);
	}
